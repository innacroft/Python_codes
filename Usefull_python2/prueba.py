...
def first_not_repeating_char(char_sequence):
    ...
    for idx, letter in enumerate(char_sequence): #crea una especie de "conteo" en idx respecto a cada letra(letter) dentro de char_sequence
        if letter not in seen_letters:  #verifica que el valor para cada letra en la secuencia inicial NO este en seen_letters(letras repetidas)
            seen_letters[letter] = (idx, 1) #si es la primera vez que encuentra la letra, crea una tupla con valores de indice de la letra y cantidad de veces vista dentro del diccionario  con key de la letra,ej:  'a': (0, 2)
            print("solo te vi una vez")
            print(seen_letters)
        else:
            seen_letters[letter] = (seen_letters[letter][0], seen_letters[letter][1] + 1) #si no es la primera vez que la ve, entonces crea otra tupla donde agrega el valor del indice en el primer lugar y el valor de veces vista en la posicion 2, este valor va aumentando cada vez que ve una palabra repetida
    ...
    if not_repeated_letters: #si la lista de not_repeated_letters no esta vacia 
        ...
        return not_repeated_letters[0][0]  #entonces retorna la posicion 0 dentro de la lista , y la posicion 0 dentro de esa tupla , ej: {('a',4),('b',6)}, en ese caso retorna a
    else:
